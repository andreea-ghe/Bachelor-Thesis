...
class PairGeometricEncoder(nn.Module):
    ...
    For each pair of parts (i, j), encodes:
        1. Center of mass: p_i = (1/N_i) sum(x_j)
        2. Pairwise distance: d_ij = RBF(||p_i - p_j||)
        3. Triplet angles: r_ij = sum_k RBF(cos ijk)
    ...
    def _compute_centroids(self, part_pcs: Tensor, n_pcs: Tensor) -> Tensor:
        """
        Compute center of mass for each part: p_i = (1/N_i) sum(x_j)
        ...
        for b in range(B):
            ...
            for p in range(P):
                n = n_pcs[b, p].item()
                if n > 0:
                    # for each part, compute the center of mass (mean) of points belonging to that part
    ...
    def _compute_triplet_angles(self, centroids: Tensor, n_pcs: Tensor) -> Tensor:
        """
        Compute triplet-wise angle features: r_ij = sum_k RBF(cos ijk)
        ...
        # sum over k: r_ij = sum_k RBF(cos ijk)
