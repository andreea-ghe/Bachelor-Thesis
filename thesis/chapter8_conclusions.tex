...
\begin{itemize}
    \item \textbf{Geometric pair attention reveals an architectural compatibility constraint.} The pair encoder, adapted from GPAT, relies on spatial relationships between fragments that evolve through iterative pose refinement. Within Jigsaw's single-pass architecture, the independent normalization applied during preprocessing limits the geometric variation available to the encoder. This finding highlights that attention mechanisms enriched with spatial priors are not freely portable across architectures---their effectiveness depends on the surrounding preprocessing and pose estimation context.
...
\paragraph{Iterative pose refinement for pair attention.} Extending Jigsaw with an iterative refinement loop---where pose estimates from each forward pass are fed back to update the geometric descriptors for the next iteration---would provide the pair encoder with progressively more informative spatial relationships, similar to the geometric recycling mechanism used in GPAT \cite{Li2023GPAT}.
