...
Yet these tasks are still carried out almost entirely by hand. Laughlin and Kelly \cite{laughlin2010lithic} found that human accuracy in lithic refitting reaches roughly 30\%, with trained specialists offering little advantage over untrained participants. Archaeological collections worldwide contain vast numbers of fragments that remain unassembled because the manual effort required far exceeds available expert time. Physical manipulation also risks further damage to already fragile objects. Together, these practical constraints have driven a sustained interest in automating the reassembly process.

With the advent of high-resolution 3D scanning (Figure~\ref{fig:case_study}), the physical pieces of a broken object can now be digitized into point clouds: unordered sets of 3D coordinates that capture the geometry of each fragment. Given a set of such point clouds, the goal is to predict the rigid transformation (a rotation and a translation) that maps each fragment back to its original position. The problem is known as 6-Degree-of-Freedom (6-DoF) pose estimation and sits at the intersection of computer vision, computational geometry, and deep learning.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/case_study_part1.png}
        \caption{Digital reconstruction of a primate statue: raw 3D scan, point cloud, and textured mesh.}
        \label{fig:case_study_reconstruction}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/case_study_part2.png}
        \caption{High-resolution structured-light 3D scanning of a cultural heritage artifact.}
        \label{fig:case_study_scanning}
    \end{subfigure}
    \caption{The digitization pipeline for cultural heritage preservation. (a)~A fractured artifact is scanned to produce point clouds and 3D meshes suitable for computational reassembly. (b)~The physical scanning process using structured-light technology.}
