...
\textbf{Fine Matching and Pose Registration.} Once coarse regions are identified, the network refines the correspondences through additional PMT layers at medium and fine resolution, using skip connections to preserve high-resolution detail. The final correspondences are extracted via Optimal Transport (Sinkhorn) \cite{cuturi2013sinkhorn} within localized clusters around the coarse matches, and a Singular Value Decomposition \cite{golub2013matrix} step computes the rigid 6-DoF transformation.
...
To model spatial relationships, the method introduces \textbf{Pair Attention}, where each pair of parts is treated as a connection in a fully connected graph. For each part, a centroid is computed, and geometric features such as distances and angles between parts are encoded using radial basis functions. These features capture how parts are positioned relative to each other.

In addition, GPAT introduces a \textbf{Point Attention} module that incorporates 6-DoF pose information directly into the attention mechanism. Each part generates a set of learnable 3D points, which are transformed using the current predicted pose. The alignment between two parts is measured by comparing the transformed points, with larger distances indicating misalignment and reducing their interaction.

The final attention score combines feature similarity, geometric relationships, and alignment penalty into a single expression. In this way, attention is guided not only by appearance but also by spatial consistency.
...
This recovery process is modeled as a continuous transformation in latent space using Rectified Flow, which progressively maps incomplete or corrupted representations into valid full-shape embeddings.

In practice, partial assemblies may be inaccurate. To mitigate this, Jigsaw++ introduces a \textbf{retargeting} step that perturbs the initial latent representation toward regions of higher probability in the latent space, then trains the model to map this perturbed latent to the ground-truth embedding.
\begin{figure}[H]
    \centering
